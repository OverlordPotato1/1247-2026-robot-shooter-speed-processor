...
struct InnerClient {
  ...
  socket_panic_receiver: parking_lot::Mutex<oneshot::Receiver<anyhow::Error>>,
...
impl Client {
  pub async fn try_new_w_config(
    ...
    let (panic_sender, panic_recv) = oneshot::channel::<anyhow::Error>();
  ...
  pub async fn try_new(server_addr: impl Into<SocketAddr>) -> anyhow::Result<Self> {
  ...
  pub async fn unpublish(&self, topic: PublishedTopic) -> anyhow::Result<()> {
    // Put message in an array and serialize
  ...
  //   to do!() // idk what this is supposed to do. i only care that it builds
  // }
  ...
  pub async fn unsubscribe(&self, sub: Subscription) -> anyhow::Result<()> {
    // Put message in an array and serialize
...
impl InnerClient {
  ...
  fn check_task_panic(&self) -> anyhow::Result<()> {
  ...
  pub(crate) async fn publish_value_w_timestamp(
    ...
    // TO DO: too lazy to handle these errors 😴  // as long as it doesn't crash i dont care
  ...
  pub(crate) async fn update_time(&self) -> anyhow::Result<()> {
  ...
  match message {
    Message::Text(message) => {
      ...
      let messages: Vec<NTMessage> = match log_result(
        serde_json::from_str(&message).map_err(|e| anyhow::Error::msg(e.to_string())),
      ) {
...
async fn setup_socket(
  ...
  panic_sender: oneshot::Sender<anyhow::Error>,
) -> anyhow::Result<()> {
